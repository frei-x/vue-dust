        ...
        add(config, originalApi) {
            ...
            notif.closeBtn && actions.push({
                label: typeof notif.closeBtn === 'string' ?
                    notif.closeBtn : this.$q.lang.label.close
            })
                ...
                if (notif.html === true) {
                    msgData.domProps = {
                        innerHTML: notif.caption ?
                            `<div>${notif.message}</div><div class="q-notification__caption">${notif.caption}</div>` : notif.message
                    }
                } else {
...
export default {
    ...
    install(vue) {
        ...
        this.create.bind(this)
            // $q.notify.setD   efaults = this.setDefaults
